...
class fw():
    def __init__(self):
        ...
        self.net = cv2.dnn.readNetFromDarknet(configFilename, weightsFilename) # this works too
        ...
        global imgInDraw, imgRef, imgMaskDiff, imgAverage
        self.imgIn = np.zeros((height, width, 3), dtype = "uint8")   # blank images
        self.imgIn2 = np.zeros((height, width, 3), dtype = "uint8")   # blank images
        ...
        global imgTemp, imgTemp2, imgTemp3, imgTemp4
        ...
        global imgMask, imgMask2, imgMask3, imgMask4
        ...
        global quadrant, th, ts, tv, lower, upper
        quadrant = 0
        th = 20
        ts = 40
        tv = 50
    ...
    def onMouseButton(self, event, x, y, flags, param): # For imgIn
    ...
    def CompwRef(self, RefClass, LiveClass):#compares ref w live to return missing ids and obstructing ids.       
        miss = ' missing: '
        obs = ' obstruction: '
        obstruction = []
        missing = []
    ...
    def keyPress(self):    
        ...
        if ((cv2.getWindowProperty(C1, cv2.WND_PROP_VISIBLE) < 1) & 
            ...
            '''
    def update(self, video_source=1):
        # # Capture Input Frame
        ...
        if ret: # ie. camera frame is available, do processing here
            ...
            self.imgIn = cv2.resize(frame, (width, height), interpolation=cv2.INTER_AREA)
            ...
            '''if click == True: # Checks when a click on imgIn is detected, extract and expand the quadrant
                if quadrant == 1: Q1 = Quadrant(self.C1, 0, 0, imgTemp, imgMask); Q1.Extract_Quadrant()
                elif quadrant == 2: Q2 = Quadrant(self.C2, int(width / 2), 0, imgTemp2, imgMask2); Q2.Extract_Quadrant()
                elif quadrant == 3: Q3 = Quadrant(self.C3, 0, int(height / 2), imgTemp3, imgMask3); Q3.Extract_Quadrant()
                elif quadrant == 4: Q4 = Quadrant(self.C4, int(width / 2), int(height / 2), imgTemp4, imgMask4); Q4.Extract_Quadrant()
                    '''
            # Averaging to minimise moving objects as part of difference calculation
            cv2.accumulateWeighted(self.imgIn3, imgAverage, 0.005) # Taking average of ImgIn3
            ...
            cv2.accumulateWeighted(self.imgin, imgAverage, 0.006)
            self.imgin = self.imgin.astype(np.uint8)
            imgAvgAoi = cv2.bitwise_and(imgInDraw, self.imgin)
            ...
            RefClassID1, RefClassID2, RefClassID3, RefClassID4 = self.detect_objects(imgRefMask)
            print('refClass3', RefClassID3)
            LiveClassID1, LiveClassID2, LiveClassID3, LiveClassID4 = self.detect_objects(imgAvgAoi)#(imgAveragewMask)
                       ...
                       # cv2.rectangle(imgIn, (x, y), (x + w, y + h), (0, 0, 255), 3)                 
                        ...
                        if (cx < (width / 2)) & (cy < (height / 2)): 
                            if RefClassID1 != LiveClassID1:
                                cv2.rectangle(self.imgIn, (0, 0), (int(width / 2), int(height / 2)), (0, 0, 255), 5)
                                cv2.rectangle(self.imgIn, (x, y), (x + w, y + h), (0, 0, 255), 3)
                                cv2.putText(self.imgIn, miss1 + obs1, (0, 5), font, 0.5, (0, 255, 255), 1) 
                        ...
                        if (cx > (width / 2)) & (cy < (height / 2)): 
                            if RefClassID2 != LiveClassID2:  
                                cv2.rectangle(self.imgIn, (int(width / 2), 0), (int(width), int(height / 2)), (0, 0, 255), 5)
                                cv2.putText(self.imgIn, miss2 + obs2, (645, 5), font, 0.5, (0, 255, 255), 1)
                                cv2.rectangle(self.imgIn, (0, int(height / 2)), (int(width / 2), int(height)), (0, 0, 255), 5)
                                #send_to_telegram("Difference detected!")
                            if RefClassID2 == LiveClassID2:
                                pass
                            if (RefClassID2 == []) & (LiveClassID2==[]):
                        ...
                        if (cx < (width / 2)) & (cy > (height / 2)): 
                            if RefClassID3 != LiveClassID3:    
                                cv2.rectangle(self.imgIn, (0, int(height / 2)), (int(width / 2), int(height)), (0, 0, 255), 5)
                                cv2.rectangle(self.imgIn, (x, y), (x + w, y + h), (0, 0, 255), 3)
                                cv2.putText(self.imgIn, miss3 + obs3, (0, 365), font, 0.5, (0, 255, 255), 1)    
                                #send_to_telegram("Difference detected!")
                            if RefClassID3 == LiveClassID3:
                                pass#print()
                            if (RefClassID3 == []) & (LiveClassID3==[]):
                        ...
                        if (cx > (width / 2)) & (cy > (height / 2)): 
                            if RefClassID4 != LiveClassID4:
                                cv2.rectangle(self.imgIn, (int(width / 2), int(height / 2)), (int(width), int(height)), (0, 0, 255), 5)
                                cv2.rectangle(self.imgIn, (x, y), (x + w, y + h), (0, 0, 255), 3)
                                cv2.putText(self.imgIn, miss4 + obs4, (645, 365), font, 0.5, (0, 255, 255), 1)
                                #send_to_telegram("Difference detected!")
                            if RefClassID4 == LiveClassID4:
                                pass#print()
                            if (RefClassID4 == []) & (LiveClassID4 == []):
                                ...
                                #send_to_telegram("Difference detected!")                    
            mainTitle = "Obstruction Detection System"
            cv2.imshow(mainTitle, self.imgIn) 
            print("update")
            #cv2.imshow('imgAvgAoi', imgAvgAoi)
            #cv2.imshow('imgRefMask', imgRefMask)z
...
class Quadrant:
    ...
    def onMouseButton_Quadrant(self, event, x, y, flags, param): # Method to draw on resized quadrant
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if quadrant == 1: drawPt = [(int(x/self.scale_percent), int(y/self.scale_percent))]
            elif quadrant == 2: drawPt = [(int((x/self.scale_percent) + (width/2)), int(y/self.scale_percent))]
            elif quadrant == 3: drawPt = [(int(x/self.scale_percent), int((y/self.scale_percent) + (height/2)))]
            elif quadrant == 4: drawPt = [(int((x/self.scale_percent) + (width / 2)), int((y/self.scale_percent) + (height/2)))]
            mouseDown2 = True
            
        elif event == cv2.EVENT_LBUTTONUP:
            mousePt2.append((x,y))
            if quadrant == 1: drawPt.append((int(x/self.scale_percent), int(y/self.scale_percent)))
            elif quadrant == 2: drawPt.append((int((x/self.scale_percent) + (width/2)),int(y/self.scale_percent)))
            elif quadrant == 3: drawPt.append((int(x/self.scale_percent),int((y/self.scale_percent) + (height/2))))
            elif quadrant == 4: drawPt.append((int((x/self.scale_percent) + (width/2)),int((y/self.scale_percent) + (height/2))))
            mouseDown2 = False
    ...
    def Extract_Quadrant(self): # Method to extract, resize and layer AOI on the extracted quadrant   
        ...
        x = int(self.x_coord)
        y = int(self.y_coord)
        crop_Quadrant = fw.imgIn2[y:y+crop_height, x:x+crop_width]
        
        # Resize the quadrant
        width_resize = int(crop_Quadrant.shape[1] * self.scale_percent)
        height_resize = int(crop_Quadrant.shape[0] * self.scale_percent)
        ...
        #cv2.imshow(self.quad_num, self.resized_quadrant)
